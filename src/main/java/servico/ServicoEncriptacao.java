 ...
 * A chave de criptografia é derivada da senha mestra do usuário e um salt persistente utilizando PBKDF2.
 * A chave e o salt são mantidos apenas na memória durante a sessão e são limpos no encerramento da JVM.
 * Uso:
 ...
 * - Use encrypt() e decrypt() para operações seguras de dados.
 * - O salt persistente é gerenciado em encryption_salt.dat.
 * Notas de Segurança:
 * - Chaves e salts são limpos da memória no encerramento da JVM via shutdown hook.
 * - AES/GCM/NoPadding é usado para criptografia, garantindo criptografia autenticada.
 */
public class ServicoEncriptacao {
	...
	// Chama o método para desligar o JVM para limpar os dados sensíveis da memória
	static {
		Runtime.getRuntime().addShutdownHook(new Thread(ServicoEncriptacao::clearSessionKeyAndSalt));
	}
	/**
